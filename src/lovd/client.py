...
def load_acquisition_config(config_path: PathLike | None = None) -> dict[str, Any]:
    ...
    if config_path:
        ...
        config_paths_to_try.extend(
            [home_config_dir / "acquisition.yaml", home_config_dir / "acquisition.yml"]
        )
...
class LOVDClient:
    ...
    def __init__(
        ...
        # Initialise private backing attributes so the property setters
        # work correctly from the very first assignment below.
        self._email: str | None = None
        self._target_gene_symbols: list[str] | None = None
        self._user_agent: str | None = None
        self._logging_level: int | None = None
        self._is_progress_enabled: bool | None = None

        # Load the YAML file once as a plain local dict.  It is used only
        # here in __init__ as a bootstrap source and is not stored on the
        # instance — use to_dict() if you need a serialisable snapshot.
        _cfg: dict[str, Any] = load_acquisition_config(config_path)
        ...
        #   3.  The value assigned to the corresponding environment
        #       variable, if set.
        #
        self.email: str = (
            email or _cfg.get("email") or EMAIL or os.getenv("LOVD_EMAIL", "")
        )
        ...
        self.target_gene_symbols: list[str] = (
            target_gene_symbols
            or list(_cfg.get("target_gene_symbols", []))
            or list(TARGET_GENE_SYMBOLS)
            or list(os.getenv("TARGET_GENE_SYMBOLS", []))
        )
        ...
        self.user_agent: str = (
            user_agent
            or _cfg.get("user_agent")
            or USER_AGENT_STRING
            or os.getenv("USER_AGENT_STRING", "")
        )
        ...
        self.logging_level: int = (
            logging_level if logging_level is not None else _cfg.get("logging_level", 2)
        )

        self.is_progress_enabled: bool = (
            is_progress_enabled
            if is_progress_enabled is not None
            else _cfg.get("is_progress_enabled", False)
        )

        # These settings have no per-attribute validation needs, so they
        # are stored as plain attributes rather than properties.
        self.save_to: PathLike | None = _cfg.get("save_to")
        self.search_terms: list[str] = _cfg.get("search_terms") or []
        self.custom_filters: dict[str, str] = _cfg.get("custom_filters") or {}
        ...
        rate_limit: int = _cfg.get("rate_limit", LOVD_RATE_LIMIT)
    ...
    def email(self, value: str | None) -> None:
        if isinstance(value, str) or value is None:
            ...
            raise TypeError(
                f"Property `email` must be of type `str`, not `{type(value)}`."
            )
    ...
    def user_agent(self, value: str | None) -> None:
        if isinstance(value, str) or value is None:
            ...
            raise TypeError(
                f"Property `user_agent` must be of type `str`, not `{type(value)}`."
            )
    ...
    @property
    def target_gene_symbols(self) -> list[str] | None:
        return self._target_gene_symbols

    @target_gene_symbols.setter
    def target_gene_symbols(self, value: list[str] | None) -> None:
        if isinstance(value, list) or value is None:
            self._target_gene_symbols = value
        else:
            raise TypeError(
                f"Property `target_gene_symbols` must be of type `list`, not `{type(value)}`."
            )

    @target_gene_symbols.deleter
    def target_gene_symbols(self) -> None:
        self._target_gene_symbols = []

    @property
    def logging_level(self) -> int | None:
        return self._logging_level

    @logging_level.setter
    def logging_level(self, value: int | None) -> None:
        if isinstance(value, int) or value is None:
            self._logging_level = value
        else:
            raise TypeError(
                f"Property `logging_level` must be of type `int`, not `{type(value)}`."
            )

    @logging_level.deleter
    def logging_level(self) -> None:
        self._logging_level = 2

    @property
    def is_progress_enabled(self) -> bool | None:
        return self._is_progress_enabled

    @is_progress_enabled.setter
    def is_progress_enabled(self, value: bool | None) -> None:
        if isinstance(value, bool) or value is None:
            self._is_progress_enabled = value
        else:
            raise TypeError(
                f"Property `is_progress_enabled` must be of type `bool`, not `{type(value)}`."
            )

    @is_progress_enabled.deleter
    def is_progress_enabled(self) -> None:
        self._is_progress_enabled = False

    def to_dict(self) -> dict[str, Any]:
        """
        Return a serialisable snapshot of the client's current configuration.

        This is the intended replacement for direct access to a ``config``
        property. The returned dictionary can be passed to ``yaml.dump()``
        to write an ``acquisition.yaml`` file that reproduces the current
        client state.

        Returns
        -------
        dict[str, Any]
            A dictionary mapping each configuration key to its current value.
    ...
    def get_variants_from_config(self) -> dict[str, dict[str, Any]]:
        ...
        return self.get_variants_for_genes(
            ...
            search_terms=self.search_terms or None,
            custom_filters=self.custom_filters or None,
        )
    ...
    def get_variants_for_genes(
        ...
        it: list[str] | tqdm[str] | None = []

        if not target_gene_symbols:
            it = self.target_gene_symbols

        if self.is_progress_enabled:
            try:
                from tqdm import tqdm

                if target_gene_symbols:
                    it = tqdm(target_gene_symbols)
                else:
                    it = tqdm(self.target_gene_symbols)
            except ImportError as e:
                if self.logging_level > 0:
                    self.logger.warning(f"Failed to import `tqdm`: {e}")
                    pass

        for gene_symbol in it:
            try:
                if self.logging_level > 0:
                    filter_desc = []
                    if search_terms:
...
def get_variants(
    ...
    # Use instance attributes as defaults, overriding with any specified parameters.
    ...
    save_to = save_to if save_to else client.save_to
    search_terms = search_terms if search_terms else client.search_terms or None
    custom_filters = custom_filters if custom_filters else client.custom_filters or None
...
def variants_to_dataframe(
    variants_data: dict[str, dict[str, Any]], is_progress_enabled: bool = False
) -> pd.DataFrame | None:
    """
